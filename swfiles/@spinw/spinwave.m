...
if param.notwin
    spectra = spinwave_incomm(obj, hkl, param);
else
    hkl  = obj.twinq(hkl);
    for ii = 1:nTwin
        % Need to pass rotC to spinwave_single to calculate
        rotC = obj.twin.rotc(:,:,ii);
        spec(ii) = spinwave_incomm(obj, hkl{ii}, param, rotC);
        if ii == 1
            spectra = spec(1);
...
function [incomm, km, nExt] = is_incomm(obj)
...
function spectra = spinwave_incomm(obj, hkl, param, varargin)
    % Run the calculations for just hkl
    spectra = obj.spinwave_single(hkl, param, varargin{:});

    % If incommensurate run calculations for hkl-km and hkl+km too
    [incomm, km, nExt] = is_incomm(obj);
    if incomm
        % Check for 2*km
        tol = param.tol*2;
        ...
        spec_m = obj.spinwave_single(bsxfun(@minus, hkl, km'), param, varargin{:});
        spec_p = obj.spinwave_single(bsxfun(@plus, hkl, km'), param, varargin{:});
